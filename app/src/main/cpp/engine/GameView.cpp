...
bool GameView::initialize() {
    ...
    for (int i = 0; i < 25; i++) {
        ...
        auto boxBound = CollisionFactory::createOBB(glm::vec3(0.0, 0.0, 0.0),
    ...
    sphere->setVolume(std::unique_ptr<SphereVolume>(
            CollisionFactory::createSphere(glm::vec3(0.0f, 0.0f, 0.0f), 0.5f)));
