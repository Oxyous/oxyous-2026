...
void OGPhysicsManager::step(float deltaTime) {
    ...
    for (auto i = 0; i < m_physicsActors.size(); ++i) {
        ...
        // Expansion margin for CCD: covers distance moved in one frame plus a base safety margin
        float speed = glm::length(physA->getVelocity());
        float ccdMargin = (speed * deltaTime) + 0.05f;
        ...
        // Track multiple static collisions but filter redundant ones (same normal)
        std::vector<OGCollisionManifold> actorStaticManifolds;
        ...
        for (const auto& poly : worldPolygons) {
            ...
            if (foundContact) {
                if (!mStatic.m_colliding) { // If not already generated by resolveCollision
                ...
                // Filtering: Only keep the deepest manifold for a given surface normal
                bool isRedundant = false;
                for (auto& existing : actorStaticManifolds) {
                    if (glm::dot(existing.m_normal, mStatic.m_normal) > 0.95f) {
                        if (mStatic.m_depth > existing.m_depth) {
                            existing = mStatic;
                        }
                        isRedundant = true;
                        break;
                ...
                if (!isRedundant) {
                    actorStaticManifolds.push_back(mStatic);
                }
            }
        }

        for (const auto& m : actorStaticManifolds) {
            m_manifolds.emplace_back(m);
        }
    }
